...
// --- CONFIGURACIÓN DE LAS URLS CON PREFIJO API ---
...
export default function DashboardHome() {
  ...
  // Extractor seguro para normalizar los datos que vengan en objetos o arrays
  ...
  const fetchStats = async () => {
    try {
      ...
      // CORRECCIÓN DE RUTAS: Agregamos /api/ para conectar con el backend real de Render
      const [rawVentas, rawProductos, rawClientes] = await Promise.all([
        fetch(`${API_URL}/api/ventas`).then(res => res.ok ? res.json() : []).catch(() => []),
        fetch(`${API_URL}/api/productos`).then(res => res.ok ? res.json() : []).catch(() => []),
        fetch(`${API_URL}/api/clientes`).then(res => res.ok ? res.json() : []).catch(() => [])
      ]);
      ...
      // Cálculo del dinero total acumulado en ventas
      const totalIngresos = listaVentas.reduce((sum, v) => {
        const valor = Number(v.total || v.total_venta || v.monto || v.precio || v.valor || v.monto_total || 0);
        return sum + valor;
      }, 0);
      ...
      // Alertas: Productos con stock menor o igual a 10 unidades
      ...
      // Top Licores: Ordenados por mayor disponibilidad en estantería
      ...
      // Top Clientes Premium: Maqueta de respaldo por si la tabla de clientes está en ceros
      ...
      console.error("Error en procesamiento de estadísticas:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const maxVentas = listadoTopProductos.length > 0 ? Math.max(...listadoTopProductos.map(p => Number(p.ventas || 0))) : 100;

  const stats = [
    { title: 'Ventas Totales', value: loading ? '...' : `$${Number(statsData.totalIngresos).toLocaleString('es-CO')}`, icon: <DollarSign size={16} className="text-emerald-600" />, bg: 'bg-emerald-50 border border-emerald-100/50' },
    { title: 'Licores en Catálogo', value: loading ? '...' : Number(statsData.totalProductos).toString(), icon: <Package size={16} className="text-amber-600" />, bg: 'bg-amber-50 border border-amber-100/50' },
    { title: 'Alertas Stock', value: loading ? '...' : listaAlertas.length.toString(), icon: <AlertTriangle size={16} className={listaAlertas.length > 0 ? "text-red-600" : "text-gray-400"} />, bg: listaAlertas.length > 0 ? 'bg-red-50 border border-red-100/50' : 'bg-gray-50 border border-gray-100' },
    { title: 'Clientes Miembros', value: loading ? '...' : Number(statsData.totalClientes).toString(), icon: <Users size={16} className="text-gray-900" />, bg: 'bg-gray-100 border border-gray-200/50' },
  ];

  return (
    <div className="space-y-5 p-1 sm:p-0 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div>
        <h1 className="text-2xl font-black text-gray-950 tracking-tight bg-gradient-to-r from-gray-950 to-amber-700 bg-clip-text text-transparent">La Cava Central</h1>
        <p className="text-gray-400 text-xs sm:text-sm mt-0.5">Perspectiva premium y analíticas en tiempo real de Licores Nicole.</p>
      </div>
                ...
                return (
                  <div key={index} className="space-y-1.5">
                    <div className="flex justify-between items-center text-xs">
                      <span className="font-bold text-gray-800 truncate max-w-[75%]"><span className="text-amber-500 font-black mr-1">#{index + 1}</span> {producto.nombre}</span>
                      <span className="font-black text-gray-950 shrink-0 bg-gray-100 px-2 py-0.5 rounded-md text-[10px] border border-gray-200/40">{stockActual} uds</span>
                    </div>
                    <div className="w-full bg-gray-100 h-2 rounded-full overflow-hidden">
                      <div className="bg-gradient-to-r from-gray-950 to-amber-500 h-full rounded-full transition-all duration-1000 ease-out" style={{ width: `${porcentaje || 1}%` }} />
                    </div>
                  </div>
                );
              })
            ...
            {loading ? (
              ...
              listadoTopClientes.map((cliente, index) => (
                <div key={index} className="flex justify-between items-center bg-gray-50/70 border border-gray-100 p-3 rounded-xl text-xs hover:bg-gray-50 transition-colors">
                  <span className="font-bold text-gray-800 truncate"><span className="text-gray-400 font-black mr-1">#{index + 1}</span> {cliente.nombre}</span>
                  <span className="font-black text-emerald-600 bg-emerald-50 px-2.5 py-1 rounded-lg text-[10px] border border-emerald-100/30">${Number(cliente.total_comprado || 0).toLocaleString('es-CO')}</span>
                </div>
              ))
            )}
          </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div className="p-4 space-y-2">
          {loading ? (
            ...
            <div className="text-center text-emerald-600 bg-emerald-50/50 border border-emerald-100/50 rounded-xl font-bold text-xs py-5">¡Todo el inventario está melo! ✨ Ningún producto en desabastecimiento.</div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              {listaAlertas.map((item, index) => (
                <div key={item.id || index} className="flex justify-between items-center bg-red-50/40 p-3 rounded-xl border border-red-100/40 text-xs hover:bg-red-50/70 transition-colors">
                  ...
                  <span className="font-black text-red-700 bg-red-100 px-2.5 py-0.5 rounded-lg text-[10px] shrink-0">{item.stock} unds restantes</span>
                </div>
              ))}
            </div>
          )}
